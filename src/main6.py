...
def run_episode(task_id: str, client: anthropic.Anthropic, verbose: bool = True) -> Dict:
    ...
    while not done:
        ...
        try:
            ...
            if verbose:
                print(f"  Step {obs['step']+1}: JSON parse error — {e}")
            # Give a small penalty step and continue
            step_log.append({"step": obs["step"]+1, "error": str(e), "reward": 0.0})
            # Force a resolve to end gracefully
            try:
                result = env.step(ResolveAction(resolution_summary="Agent parse error", satisfied=False))
                done = True
            except Exception:
                break
